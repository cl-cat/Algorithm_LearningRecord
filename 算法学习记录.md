...
int main(){
    ...
    while(m--){
        ...
        else if(k==2){ // 表示树从 x 到 y 节点最短路径上所有节点的值的和
            cin >> x >> y;
            printf("%d\n",qRange(x,y));
        }
...
> $$
> \begin{cases} x_{c_1} - x_{c_1^′} \leq y_1 \\
> x_{c_2} - x_{c_2^′} \leq y_2 \\
> \dots \\
> x_{c_m} - x_{c_m^′} \leq y_m \end{cases}
> $$
...
>
> 1. 对于操作1，格式为 $v_i \text{ } 1 \text{ } loc_i \text{ } value_i$ ，即为在版本 $v_i$ 的基础上，将 $a_{loc_i}$ 修改为 $value_i$ 。
> 2. 对于操作2，格式为 $v_i \text{ } 2 \text{ } loc_i$ ，即访问版本 $v_i$ 中的 $a_{loc_i}$ 的值。
...
> 给出一个长度为 $n$ 的数列，$a_1 ,a_2 ,\dots,a_n$ ，有 $q$ 个询问，每个询问给出数对 $(i,j)$ ，需要你给出 $a_i, a_
> {i + 1},\dots , a_j$ 这一段中有多少不同的数字。
